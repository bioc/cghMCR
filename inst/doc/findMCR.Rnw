...
The sample data are stroed in the \textit{data} subdirectory and can be read in using \Rfunction{read.Agilent} of \Rpackage{marray}.

<<>>=
require("cghMCR")
require("marray")
sampleData <- read.Agilent(list.files(file.path(.path.package("cghMCR"), 
                  "sampledata"), full.name = TRUE, pattern = "sample"), 
                  path = "")
@
...
It normally takes a few steps such as quality assessment and normalization before engaging data analysis. Here we conduct a loess normliztion that is supported by \Rfunction{marray}. Readers are referred to packages \Rpackage{marray} and \Rpackage{limma} for further information.
